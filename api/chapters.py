...
from api.duels import get_match_for_session, record_duel_strike
...
def complete_quest_route(
    ...
    # Duel carve-out (BACKEND_INTEGRATION.md 2.9.6): a duel-bound session
    # never fires the goblin and stays "battle" regardless of its own
    # local bossHp math -- the quest's damage instead hits the opponent
    # via the match, which is what actually decides the duel.
    duel_match = get_match_for_session(session_id)
    if duel_match is not None:
        updated["status"] = "battle"
        goblin = None
        duel_match = record_duel_strike(session_id, quest["damage"]) if not was_done else duel_match

    if not was_done:
        _archive_quest_for_similarity(storage, {**quest, "done": True, "damage": quest["damage"]})
    storage.save_chapter(session_id, updated)

    if duel_match is None and updated["status"] == "chapterEnd" and session["status"] != "chapterEnd":
    ...
    response = {"state": updated, "goblin": goblin}
    if duel_match is not None:
        response["match"] = duel_match
    return response
