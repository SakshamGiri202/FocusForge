          ...
          <motion.div
            ...
            className={`group flex items-center gap-3 rounded-lg border px-4 py-3 transition-colors ${
              isDone
                ...
                : "border-gold-600/30 bg-ink-900/70 hover:border-gold-500/50"
            }`}
          >
            ...
            {isDone ? (
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                {fighting && !busy && (
                  <button
                    type="button"
                    onClick={() => onStrike(q.id)}
                    disabled={busy}
                    className="rounded-md border border-gold-500/60 bg-gold-500/10 px-3 py-1 text-sm text-gold-200 transition-all hover:bg-gold-500/25 hover:shadow-[0_0_18px_rgba(201,162,39,0.35)] disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    ⚔ strike
                  </button>
                )}
